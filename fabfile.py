...
Base configuration
"""
env.project_name = 'gradereports'
env.database_password = 'DkPnLtjS9Q'
...
def production():
    ...
    env.hosts = ['data.apps.dailyillini.com']
    env.user = 'projects'
    env.s3_bucket = 'media.apps.dailyillini.com'
...
def load_data():
    ...
    local("scp %(localpath)s/data/dump.sql.gz %(user)s@data.apps.dailyillini.com:data/%(project_name)s" % env)
